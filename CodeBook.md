# Data Introduction
- This project will use six data, which are `x_train.txt`, `x_test.txt`, `y_train.txt`, `y_test.txt`, `subject_train.txt` and `subject_test.txt`, they can all be found inside the downloaded dataset, namely **UCI HAR Dataset**.
...
- The `README.txt` is the overall desciption about the overall process of how publishers of this  did the experiment and got the data result.


# Course Project Introduction
The script `run_analysis.R` uses the `dplyr` package for renaming column and reading in files. It performs 5 major steps including:
...
- The column names are given of the real names '`Gravity' 
- The first column refers to each subject that did the experiment. 
- Column contain the feature variables(mean and std of the whole feature variables).
